    ...
    <div className="overflow-hidden rounded-lg shadow-md border border-gray-100">
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-brandRose-light/50">
            <tr>
              {columns.map((col, index) => (
                <th 
                  key={col || index} 
                  className="px-6 py-3 text-left text-xs font-medium text-brandRose-dark uppercase tracking-wider"
                >
                  {col}
                </th>
              ))}
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <button
                    ...
                    className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${
                      currentPage === page
                        ? 'z-10 bg-brandRose border-brandRose text-white'
