...
public class UserController {
     ...
     * 2. userService.findAll() retrieves data
     * 3. Spring converts List<User> to JSON
     * 4. Wrapped in 200 OK response
     */
    @GetMapping
    public ResponseEntity<List<User>> getAllUsers() {
        return ResponseEntity.ok(userService.findAll()); // ResponseEntity: Wraps responses with HTTP status codes and headers
    }

    @GetMapping("/{id}")
    public ResponseEntity<User> getUserById(@PathVariable UUID id) { // @PathVariable extracts {id} from URL and converts to UUID
